...
%and apply a smoothing spline to remove noise from the trjaectory
...
function DS = ssplineTransXML(toolNames,fileName)
...
for i=1:length(D)
    DS{i} = D{i}.movingAverageTrans(PC.get('Sspline'),o.read('Q'));
    writeRecord(DS{i});
end%for
...
DS = textToXML([fileName, '_SsplineTrans'],toolNames,procNums);