...
%style file with timestamps repeated 5 times (for use in the
%PerkProcedureEvaluator module, which steps by 5 timestamps)
...
function status = DataToAscTrack5(D,outFile,toolNames)
...
while (moreTime)
    ...
    %Now, add the data from the minimum tool to the xml file 5 times
    for five=1:5        
        ...
        docRootNode.appendChild(thisElement); %1
    end%for
