...
function [Sty_Tr Pro_Tr Ref_Tr] = NDITrackToDataTask(subjNum,trial,skill,technique)
...
[transT transK] = readManSegEP(segFile,trial);
K = segToTaskData(T,transT,transK);
...
chop = (K ~= 0);
T = T(chop);
X_Sty_Tr = X_Sty_Tr( chop, : );
X_Ref_Tr = X_Ref_Tr( chop, : );
X_Pro_Tr = X_Pro_Tr( chop, : );
K = K(chop);
