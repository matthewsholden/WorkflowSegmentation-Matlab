...
%segmentation algorithm (training error calculation)
...
function [acc D_Test] = trainErrEpiduralType(skill,technique)
...
onlyTrain = ones(subjNum);
for i=1:subjNum
    D_Test{i} = Data( D{i}.T, D{i}.X, zeros(size(D{i}.K)), D{i}.S );
    D_Train(:,i) = D(~~onlyTrain(:,i)); % IMPORTANT: ~~ is required!   
end%for
