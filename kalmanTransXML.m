...
%and apply a Gaussian filter to remove noise from the trjaectory
...
%Return DS: A cell array of the kalman filtered data for each tool
function DS = kalmanTransXML(toolNames,fileName)
...
for i=1:length(D)
    DS{i} = D{i}.kalmanTrans(PC.get('Kalman'),o.read('Q'));
    writeRecord(DS{i});
end%for
...
DS = textToXML([fileName, '_KalmanTrans'],toolNames,procNums);